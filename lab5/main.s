...
# options passed: -mtune=generic -march=x86-64 -fasynchronous-unwind-tables -fstack-protector-strong -fstack-clash-protection -fcf-protection
	.text
# int   init_gv1 = 10;
	.globl	init_gv1				#可见性声明
	.data							#初始化后的全局变量放在.data段
	.align 4						#布局信息（数据对齐）
	.type	init_gv1, @object		#类型：数据对象
	.size	init_gv1, 4				#大小（占用字节数）
init_gv1:							#符号地址标记
	.long	10						#数据类型和值

# int   uninit_gv2;
	.globl	uninit_gv2				#符号声明
	.bss							#未初始化的全局变量放在.bss段
	.align 4						#布局信息（对齐）
	.type	uninit_gv2, @object		#类型：数据对象
	.size	uninit_gv2, 4			#大小（占用字节数）
uninit_gv2:							#符号地址标记
	.zero	4						#未被初始化，仅分配空间
	.globl	init_gstr1
	.data
...
# main.c:32:     printf("auto x = %d , static count =%d\n",x,count);
	movl	count.0(%rip), %edx	#传参数count
	movl	-4(%rbp), %eax	# x, tmp85
	movl	%eax, %esi	# //传参数x
	leaq	.LC0(%rip), %rax	#, tmp86
	movq	%rax, %rdi	# 	//传参数字符串"auto x = %d , static count =%d\n"
	movl	$0, %eax	#	//不需要使用向量寄存器
...
.LFB1:
	...
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp	# 将栈顶作为新的基地址
	.cfi_def_cfa_register 6
	subq	$80, %rsp	# 开80个字节的栈空间
	movl	%edi, -68(%rbp)	# x, x 局部变量（参数）x存入栈
	movl	%esi, -72(%rbp)	# y, y 局部变量（参数）y存入栈
# main.c:36: {
...
.LFB2:
	...
	.cfi_def_cfa_register 6
	subq	$16, %rsp	#,
# main.c:48:      check_gcc_condition(init_gstr1);   //检查学号与条件编译是否一致，该函数没有返回值
	leaq	init_gstr1(%rip), %rax	#
	movq	%rax, %rdi	# 将参数放入寄存器rdi中
	call	check_gcc_condition@PLT	# 调用函数
# main.c:49:      fcount( );    //该函数没有参数和返回值
	movl	$0, %eax	# printf函数不需要用到向量寄存器
	call	fcount	# 调用函数
# main.c:50:      fcount( );
	movl	$0, %eax	#,
	call	fcount	#
...
#NO_APP
	movl	$15, -12(%rbp)	# 将lv1存入栈
# main.c:54:      int  lv2 = 17;
	movl	$17, -8(%rbp)	# 将lv2存入栈
# main.c:55:      int  sum = 0;
	movl	$0, -4(%rbp)	# 将sum存入栈
...
# main.c:59:      strcpy(uninit_gstr2,"qyy");   // 请换成自己的姓名拼音
	movl	$7960945, uninit_gstr2(%rip)	#, MEM <char[1:4]> [(void *)&uninit_gstr2]
